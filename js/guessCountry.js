...
let latToGuess;
let lonToGuess;
seqOrdered = ["a", "b", "c", "d", "e", "f"];
seqOrdered = shuffle(seqOrdered);
//console.log(seqOrdered);

function flag(j) {
	let flag = document.querySelector("#flag");
	console.log(countryList.country[j].ISO2);
	flag.src = `https://flagcdn.com/${countryList.country[
		j
...
function getCoord() {
	for (let i of coordinates) {
		if (i["Alpha-2 code"] === countryList.country[j].ISO2) {
			console.log(i);
			latToGuess = i["Latitude (average)"];
			lonToGuess = i["Longitude (average)"];
		}
	}
}
...
inputBox.addEventListener("keydown", (event) => {
	...
	if (
		...
		event.key === "Enter" &&
		arrayCountry.indexOf(inputBox.value) != -1 
		&&
		!(guessedA.indexOf(inputBox.value) != -1)
	) {
		console.log("ARRAY",guessedA);

		event.preventDefault(); 	
		console.log(inputBox.value);
		let guessed = inputBox.value;
		console.log(countryList.country[j].Name);

		if (
			inputBox.value.toLowerCase().trim() ===
			countryList.country[j].Name.toLowerCase()
		) {
			...
			setTimeout(() => {
				const modalElem = modal(true, countryList.country[j].Name);
				const center = document.querySelectorAll("#try");
				center[center.length -1 ].after(modalElem);
			...
			if(guessR === 6){
				setTimeout(() => {
					const modalElem = modal(false, countryList.country[j].Name);
					const center = document.querySelectorAll("#try");
					center[center.length -1 ].after(modalElem);
					modalElem.style.display = "block";
				}, 2000);
			}
			console.log(`#${seqOrdered[guessR - 1]}`);
			let box = document.querySelector(`#${seqOrdered[guessR - 1]}`);
			...
			let lon = 0;
			for (let i of coordinates) {
				if (i.Country.toLowerCase() === guessed.toLowerCase()) {
					lat = i["Latitude (average)"];
					lon = i["Longitude (average)"];
				}
			}
			cell1.innerHTML = inputBox.value;

			let distance = vincentyDistance(lat, lon, latToGuess, lonToGuess);
			let pend = bearingBetweenPoints(lat, lon, latToGuess, lonToGuess);
			cell2.innerHTML = distance.toFixed(2) + " Km";
...
const angle = (lat1, lon1, lat2, lon2) => {
	...
	const cosDelta = Math.cos(lon2ToRad - lon1ToRad);

	const cosAlpha = Math.cos(sinLat1 * sinLat2 + cosLat1 * cosLat2 * cosDelta);

	const cgamma = Math.PI / 2 - lat2ToRad;
	const sinc = Math.sin(cgamma);

	const sinA = Math.sin(lon2ToRad - lon1ToRad);

	const alpha = Math.acos(cosAlpha);
	const sinAlpha = Math.sin(alpha);

	const sinC = (sinA * sinc) / sinAlpha;
...
const direction = (lat1, lon1, lat2, lon2) => {
	...
	const ret =
		...
		latDelta < -9 && lonDelta > 9
			...
			latDelta > 9 && lonDelta < -9
			? emoji.SE
			: latDelta > 0 && latDelta < 9 && lonDelta > 0
			? emoji.N
			: // Sud Case
			latDelta > -9 && latDelta < 9 && lonDelta < 0
			? emoji.S
			: // West Case
			lonDelta > -9 && lonDelta < 9 && latDelta < 0
			? emoji.W
			: //Est Case
			lonDelta > -9 && lonDelta < 9 && latDelta > 0
			? emoji.E
...
const bearingBetweenPoints = (lat1, lon1, lat2, lon2) => {
	...
	inp.addEventListener("keydown", function (e) {
		...
		if (e.keyCode == 40) {
          ...
          decrease the currentFocus variable:*/
			...
			/*and and make the current item more visible:*/
			addActive(x);
		} else if (e.keyCode === 9 || e.keyCode === 32 || e.keyCode === 13) {
			e.preventDefault();
			inputBox.value = x[currentFocus].querySelector("input").value;
...
const modal = (isGuessed, name) => {
	let modal = document.createElement("div");
	modal.id = "winModal";
	modal.classList.add("modal");

	let modalContent = document.createElement("div");
	modalContent.id = "modalConten";
	modalContent.classList.add("modal-content");

	let closeBtn = document.createElement("span");
	closeBtn.id = "modalBtn";
	...
	p1.innerHTML = isGuessed ? "Congratulations" : `Damn the country was: ${name}`;
	p2.innerHTML = isGuessed
	...
	image.src = "./assets/HomeCode.png";

	let footer = document.createElement("div");
	footer.classList.add("modal-footer");
	footer.id = "Btn";
	let footerBtn = document.createElement("buttone");
	footerBtn.type = "button";
	footerBtn.classList.add("btn");
	footerBtn.classList.add("btn-primary");
	footerBtn.classList.add("center");
	footerBtn.id = "btnNewGame";
	footerBtn.innerHTML = "New game";
    footerBtn.onclick = () => {
        location.reload();
    }
	...
	modalContent.append(textContainer);
	textContainer.append(p1);
	textContainer.append(p2);
	isGuessed ? null : modalContent.append(imageContainer);
	...
	modal.addEventListener('click', (event) => {
	  ...
	  modal.addEventListener('hover', (event) => {
        if (event.target != modal) {
          event.stopPropagation();
        }
      });
